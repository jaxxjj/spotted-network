...
func NewStateOnlyClient(rpcEndpoint string, stateManagerAddr common.Address) (contracts.StateClient, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[StateOnlyClient] failed to connect to Ethereum node: %w", err)
	}
	...
	if err != nil {
		ethClient.Close()
		return nil, fmt.Errorf("[StateOnlyClient] failed to create state manager binding: %w", err)
	}
...
func (c *StateOnlyClient) GetStateAtBlock(ctx context.Context, target common.Address, key *big.Int, blockNumber uint64) (*big.Int, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[StateOnlyClient] failed to get state at block: %w", err)
	}
...
func (c *StateOnlyClient) GetStateAtTimestamp(ctx context.Context, target common.Address, key *big.Int, timestamp uint64) (*big.Int, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[StateOnlyClient] failed to get state at timestamp: %w", err)
	}
	if len(history) == 0 {
		return nil, fmt.Errorf("[StateOnlyClient] no state found after timestamp")
	}
...
func (c *StateOnlyClient) GetLatestState(ctx context.Context, target common.Address, key *big.Int) (*big.Int, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[StateOnlyClient] failed to get latest state: %w", err)
	}
	return value, nil
}
...
func (c *StateOnlyClient) GetLatestBlockNumber(ctx context.Context) (uint64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("[StateOnlyClient] failed to get latest block number: %w", err)
	}
