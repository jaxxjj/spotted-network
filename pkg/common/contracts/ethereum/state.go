...
func (c *StateClient) GetStateAtBlock(ctx context.Context, target common.Address, key *big.Int, blockNumber uint64) (*big.Int, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[StateClient] failed to get state at block: %w", err)
	}
...
func (c *StateClient) GetStateAtTimestamp(ctx context.Context, target common.Address, key *big.Int, timestamp uint64) (*big.Int, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[StateClient] failed to get state at timestamp: %w", err)
	}
	if len(histories) == 0 {
		return nil, fmt.Errorf("[StateClient] no state found at timestamp %d", timestamp)
	}
...
func (c *StateClient) GetLatestBlockNumber(ctx context.Context) (uint64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("[StateClient] failed to get latest block number: %w", err)
	}
	return blockNumber, nil
}

// GetEthClient returns the underlying ethclient.Client
func (c *StateClient) GetEthClient() *ethclient.Client {
	return c.ethClient
}
